    ...
    public static void main(String[] args) {
        {
            ...
            if (N % workersCount != 0) {
                if (currentProcess == MASTER) {
                    System.out.println("It is impossible to allocate the specified number of calls to "
            ...
            if (currentProcess == MASTER) {
                ...
                for (int dest = 1; dest <= workersCount; dest++) {
                    ...
                    double[] subMatrixA1D = MatrixTools.convert2DTo1D(Arrays.copyOfRange(matrixA, startRow, endRow));
                ...
                for (int source = 1; source <= workersCount; source++) {
                    ...
                    MatrixTools.fillMatrixWithOffset(matrixC, resultMatrix1D, offset[0] * rowsPerProcess);
                }
...
//                MatrixTools.printMatrix(matrixC);
                ...
                double[][] subMatrixA = MatrixTools.convert1DTo2D(subMatrixA1D, rowsPerProcess, N);
                ...
                MPI.COMM_WORLD.Send(MatrixTools.convert2DTo1D(resultMatrix), 0, rowsPerProcess * N, MPI.DOUBLE, MASTER, FROM_WORKER + 1);
            }
...
//                MatrixTools.printMatrix(matrixC);
                ...
                MPI.COMM_WORLD.Isend(MatrixTools.convert2DTo1D(resultMatrix), 0, rowsPerProcess * N, MPI.DOUBLE, MASTER, FROM_WORKER + 1);
            }
